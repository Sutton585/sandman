...
new_nom_to_ai = """  Your task is to maintain a hyper-organized, easily navigable nomenclature dictionary that translates Sandman's universal orchestration variables into the specific parameters used by underlying modules (like JobSpy and Digestitor).
  ...
  2. Clearly list what each standard variable maps to in JobSpy (job2markdown) and Digestitor (reddit2markdown).
  3. Maintain the standardized markdown front-matter schema for all generated text files.
  
  How to accomplish it:
  - Do not use bold markdown formatting anywhere in this file.
  ...
  Left to do:
  Add equivalent mappings for upcoming modules (like gmail2markdown or Facebook Marketplace scrapers) as they are integrated into the Sandman ecosystem."""

new_nom_body = """
# Sandman Universal Nomenclature Standard

This document establishes the unified naming conventions across the Sandman ecosystem. Sandman orchestration translates these universal standard variables down into the specific parameters required by individual modules (like Digestitor for Reddit, or job2markdown using JobSpy). 
...
* query
  ...
  * Maps to Digestitor: N/A (Digestitor relies on source instead of search query)

* source
  * Used for: The platform, sub-community, or job board being targeted.
  * Maps to JobSpy: site_name (e.g., linkedin, indeed)
  * Maps to Digestitor: subreddit (must strip out any r/ or /r/ prefixes)
...
* group_by_source
  ...
  * Maps to Digestitor: Subreddit Folders

* location
  * Used for: Geographic search area.
  * Maps to JobSpy: location
  * Maps to Digestitor: N/A
...
* min_age_hours
  ...
  * Maps to Digestitor: Minimum Post Age Hours
...
* blacklist_terms
  ...
  * Maps to Digestitor: Filter Keywords

* blacklist_urls
  * Used for: Domains or URL fragments to exclude.
  * Maps to JobSpy: Filtered by Sandman wrapper
  * Maps to Digestitor: URL Blacklist
...
* proxies
  ...
  * Maps to Digestitor: Handled by Sandman wrapper if needed

* user_agent
  * Used for: Overriding the default browser user agent.
  * Maps to JobSpy: user_agent
  * Maps to Digestitor: Handled by Sandman wrapper
...
* sort
  ...
  * Maps to JobSpy: N/A
  * Maps to Digestitor: Reddit Sort Method (new, hot, top, rising)
...
* module
  * Description: The specific Sandman module that executed the scrape (e.g. job2markdown, digestitor).
